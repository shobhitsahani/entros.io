...
The current published version is `1.5.0`. Install with `npm install @entros/pulse-sdk`. See the [changelog](/docs/reference/changelog) for the upgrade path from earlier versions.
...
| `submitViaRelayer` | `(proof, commitment, options) => Promise<SubmissionResult>` | Submit through the executor's relayer (walletless mode) |
...
| `fetchChallenge` | `(walletAddress, relayerUrl, apiKey?) => Promise<ChallengeResponse>` | Fetch a challenge from the executor |
...
| `attestAgentOperator` | `(agentAsset, options) => Promise<AttestResult>` | Write the human-operator metadata for a registered agent |
| `getAgentHumanOperator` | `(agentAsset, connection) => Promise<AgentHumanOperator \| null>` | Read the human-operator metadata. `AgentHumanOperator` fields: `{ anchorPda, trustScore, verifiedAt, wallet }` |
